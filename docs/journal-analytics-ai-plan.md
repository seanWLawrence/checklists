# Journal Analytics AI MVP Plan

## Goal
Let a user ask basic AI questions about `src/app/journals/analytics/[since]/page.tsx` using the smallest useful analytics payload.

Primary MVP use case:
- `Summarize this period`

Nice-to-have after MVP:
- simple ad hoc questions like `What activities seem best for my mood?`

---

## MVP Scope
Build only what is needed for a basic, grounded AI summary flow:
...
For MVP, skip the planner entirely and use preset plans in code.

---

## Required Data Slices
Keep the slice system small.
...
- `entryRows`: compact row-level data for later non-MVP questions

For the first merge, only `overview` is strictly required. `sentimentTimeline` / `activityImpact` / `helpfulActivities` can be added if already easy from existing analytics logic. `entryRows` is optional for MVP but useful groundwork.
...
## `overview` Slice
This is the most important slice. It should answer most summary prompts with low token cost.
...
If implementation speed matters, start with an even smaller plan:
...
No ad hoc input required for first merge.
...
- never include full journal body in MVP
...
Add `entryRows` support only if needed for the next question type.
...
6. Add more slices only if needed

Do **not** build planner or trace persistence until the preset flow is working.

---

## Definition of Done
The first merge is done when:
- app can request analytics slices without using page JSX
...
1. add ad hoc question input
2. add planner model for custom questions
3. add trace persistence
4. add review/debug UI
5. add compare-period support
